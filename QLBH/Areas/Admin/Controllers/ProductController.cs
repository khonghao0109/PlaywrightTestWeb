...
namespace MyWebApp.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Edit(ProductModel product)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (product.ImageUpload != null)
                {
                    ...
                    {
                        ModelState.AddModelError("", "Am error occurred ");
                    }
                    FileStream fs = new FileStream(filePath, FileMode.Create);
                    await product.ImageUpload.CopyToAsync(fs);
                    fs.Close();
        ...
        public async Task<IActionResult> Delete(long Id)
        {
            ...
            try
            {
                // Kiểm tra nếu sản phẩm có hình ảnh và không phải ảnh mặc định thì xóa ảnh
            ...
            {
                if (ex.InnerException is SqlException sqlEx && sqlEx.Number == 547) // 547: Lỗi khóa ngoại
                {
                    TempData["error"] = "Không thể xóa sản phẩm vì đã có đơn hàng chứa sản phẩm này.";
                }
                else
                {
